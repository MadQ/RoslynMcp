...
Every `roslyn_*` tool takes a `projectPath`, so one running server can serve multiple projects without a restart. Highlights from the 45 tools (43 public + 2 debug-only):
