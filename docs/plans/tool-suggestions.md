...
Given a method, list every method it calls (direct calls only, one level deep). Roslyn's `IOperation` tree makes this precise — you walk `IInvocationOperation` nodes in the method body. Essential for "what does this method depend on?" without reading it. Pair with `roslyn_find_callers` for full dependency tracing.
...
**`roslyn_read_file`** — add a `symbolName` parameter: "give me the lines containing `ParseDocumentation`." Internally does the same work as `roslyn_get_member_body` but surfaces it as a read operation. Lets you skip the line-number lookup step entirely.
