...
| `CheckDriftTool` | `roslyn_check_drift` — workspace health probe on two axes: source drift (on-disk changes the FileSystemWatcher missed) and reference health (`workspace_healthy: false` when a project loaded with zero metadata references). Use when symbol results look stale **or** when `roslyn_get_diagnostics` reports errors you don't believe |
