...
Without this, subsequent Roslyn tools see the stale in-memory source tree, not the updated file. `InvalidateFile` classifies the path via `GetDocumentIdsWithFilePath` + a `GetDocument`/`GetAdditionalDocument`/`GetAnalyzerConfigDocument` check: a tracked `.cs` document *or* a tracked `AdditionalFiles` item is applied incrementally (`Solution.WithDocumentText`/`WithAdditionalDocumentText`, #276 — `MSBuildWorkspace.CanApplyChange` supports both); a new `.cs`, a new `AdditionalFiles` item, an MSBuild evaluation input (`.csproj`, `.props`, `.targets`, `Directory.Build.*`, `global.json`, `nuget.config`, `packages.lock.json`, `.resx`), or a tracked analyzer-config document (`.editorconfig`, `.globalconfig` — `CanApplyChange(ChangeAnalyzerConfigDocument)` is `false` as of Roslyn 5.3.0, so these always reload even when already tracked) flags a full reload on the next compilation-needing call, logged as `Reload — Flagged (<reason>): <path>`; anything else (`.md`, `.txt` not declared as an `AdditionalFiles` item, an unrelated `.json`) is a no-op for the workspace, so calling it is always cheap and always correct. See `docs/development/WORKSPACE_SYNC.md`.
