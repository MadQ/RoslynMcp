...
- **Development console output polish** — server startup output now shows the resolved workspace mode, discovery method (SDK / VS / pin), and any applied environment-variable overrides (e.g. MSBuild path, load timeout, elicit mode) for transparency during debugging. Quiet mode is unchanged (`--quiet` / `ROSLYNMCP_QUIET=true` still suppresses startup noise). (#246)
