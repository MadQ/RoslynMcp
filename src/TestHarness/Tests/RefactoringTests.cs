...
internal static class RefactoringTests
{
	internal static TestGroup Build(TestContext ctx)
	{
		...
		// Dedicated fixtures for the mutation-boundary and workspace-binding regression tests below —
		// declared up front (before the first tool call) like every other fixture in this group, so
		// no test depends on the FileSystemWatcher's debounce window.
		var staleRenamePath = fx.PathOf("_StaleRenameFixture_.cs");
		fx.Write("_StaleRenameFixture_.cs", """
			// AUTO-GENERATED by TestHarness — safe to delete.
			namespace RoslynMcp
			;
			
			class _StaleRenameFixture_
			{
				public void Placeholder() { }
			}
			""");
		
		fx.Write("_MismatchRenameFixture_.cs", """
			// AUTO-GENERATED by TestHarness — safe to delete.
			namespace RoslynMcp
			;
			
			class _MismatchRenameFixture_
			{
				public void Placeholder() { }
			}
			""");
		
		var staleSigChangePath = fx.PathOf("_StaleSigChangeFixture_.cs");
		fx.Write("_StaleSigChangeFixture_.cs", """
			// AUTO-GENERATED by TestHarness — safe to delete.
			namespace RoslynMcp
			;
			
			class _StaleSigChangeFixture_
			{
				public void Method(string x) { }
			}
			""");
		
		fx.Write("_MismatchSigChangeFixture_.cs", """
			// AUTO-GENERATED by TestHarness — safe to delete.
			namespace RoslynMcp
			;
			
			class _MismatchSigChangeFixture_
			{
				public void Method(string x) { }
			}
			""");
		...
		// Regression coverage for #264: PhysicalSolutionApplier's TOCTOU guard must catch a file
		// mutated on disk between preview and apply, rather than silently clobbering the concurrent
		// edit — the practical, harness-achievable form of "forced mutation-boundary fault" coverage
		// (true filesystem/antivirus-level truncation isn't reproducible from the harness, per the
		// note in EditingTests.cs).
		async Task<(bool pass, string msg)> RunRenameStalePreview()
		{
			...
			var (_, pvData, _) = await Call("roslyn_preview_rename", new {
				...
				symbolName  = "_StaleRenameFixture_",
				newName     = "_StaleRenameFixtureRenamed_",
				projectPath = fx.Csproj
			});
			...
			await File.WriteAllTextAsync(staleRenamePath, "// concurrent edit\n");
			
			var (_, apData, _) = await Call("roslyn_apply_rename", new {
				token, approval = "y", projectPath = fx.Csproj
			});
			
			var pass = apData?["error"]?.GetValue<string>() == "stale preview";
			
			return (pass, pass ? $"PASS  [{sw.ElapsedMilliseconds}ms]" : $"FAIL  (error: {apData?["error"]}) [{sw.ElapsedMilliseconds}ms]");
		}
		
		// Regression coverage for #264: the workspace-binding guard rejects an apply whose projectPath
		// resolves to a different workspace than the one the preview was created against, and leaves
		// the token pending so a retry against the correct workspace still succeeds.
		async Task<(bool pass, string msg)> RunRenameWorkspaceMismatch()
		{
			...
			var (_, pvData, _) = await Call("roslyn_preview_rename", new {
				...
				symbolName  = "_MismatchRenameFixture_",
				newName     = "_MismatchRenameFixtureRenamed_",
				projectPath = fx.Csproj
			});
			...
			var (_, mismatchData, _) = await Call("roslyn_apply_rename", new {
				token, approval = "y", projectPath = ctx.TargetPath
			});
			
			var (_, retryData, _) = await Call("roslyn_apply_rename", new {
				token, approval = "y", projectPath = fx.Csproj
			});
			
			var pass = mismatchData?["error"]?.GetValue<string>() == "workspace mismatch"
				&& retryData?["error"] is null
				&& retryData?["filesWritten"]?.GetValue<int>() >= 1;
			
			return (pass, pass
				? $"PASS  [{sw.ElapsedMilliseconds}ms]"
				: $"FAIL  (mismatch: {mismatchData?["error"]}, retry error: {retryData?["error"]}) [{sw.ElapsedMilliseconds}ms]");
		}
		
		async Task<(bool pass, string msg)> RunSigChangeStalePreview()
		{
			...
			var (_, pvData, _) = await Call("roslyn_change_signature", new {
				...
				methodName     = "Method",
				containingType = "_StaleSigChangeFixture_",
				addParameters  = """[{"name":"y","type":"int","defaultValue":"0"}]""",
				projectPath    = fx.Csproj
			});
			...
			await File.WriteAllTextAsync(staleSigChangePath, "// concurrent edit\n");
		...
		async Task<(bool pass, string msg)> RunSigChangeWorkspaceMismatch()
		{
			...
			var (_, pvData, _) = await Call("roslyn_change_signature", new {
				...
				methodName     = "Method",
				containingType = "_MismatchSigChangeFixture_",
				addParameters  = """[{"name":"y","type":"int","defaultValue":"0"}]""",
				projectPath    = fx.Csproj
			});
			...
			var (_, mismatchData, _) = await Call("roslyn_apply_signature_change", new {
				token, approval = "y", projectPath = ctx.TargetPath
			});
			
			var (_, retryData, _) = await Call("roslyn_apply_signature_change", new {
				token, approval = "y", projectPath = fx.Csproj
			});
		...
		var tests = new List<TestCase> {
			...
			new("roslyn_apply_rename: consumed token returns not found",
				async () => {
					...
					// Same token must now return "unavailable" — the 3-state approval lifecycle (#264)
					// removes the token from pending on reject, same as it does once a physical apply begins.
					var (_, apData, _) = await Call("roslyn_apply_rename", new {
						token, approval = "y", projectPath = ctx.TargetPath
					});
					var pass = apData?["error"]?.GetValue<string>() == "token unavailable";
			...
			new("roslyn_apply_rename: unknown token returns error",
				() => ctx.RunTestAsync(
					...
					data => data?["error"]?.GetValue<string>() == "token unavailable")),
			
			new("roslyn_apply_rename: file renamed when class name matches stem",
				() => RunFileRename()),
			
			new("roslyn_apply_rename: stale preview (#264) is rejected, token stays valid",
				() => RunRenameStalePreview()),
			
			new("roslyn_apply_rename: workspace mismatch (#264) is rejected, token stays valid",
				() => RunRenameWorkspaceMismatch()),
			...
			new("roslyn_apply_signature_change: stale preview (#264) is rejected, token stays valid",
				() => RunSigChangeStalePreview()),
			
			new("roslyn_apply_signature_change: workspace mismatch (#264) is rejected, token stays valid",
				() => RunSigChangeWorkspaceMismatch()),
