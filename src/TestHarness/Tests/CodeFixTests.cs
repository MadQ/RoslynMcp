...
internal static class CodeFixTests
{
	internal static TestGroup Build(TestContext ctx)
	{
		...
		async Task<(bool pass, string msg)> RunRejectStaleFix()
		{
			var fixturePath = Path.Combine(ctx.RepoRoot, "src", "RoslynMcp", "_CodeFixFixture_.cs");
			var sw = Stopwatch.StartNew();
			...
			try {
				...
				if(token is null)
					return (false, $"FAIL  (preview missing token: {pvText}) [{sw.ElapsedMilliseconds}ms]");
				
				const string concurrentEdit = "// Concurrent user edit must survive.\n";
				...
				var error = apData?["error"]?.GetValue<string>();
				var updated = await File.ReadAllTextAsync(fixturePath);
				var pass = error == "stale preview" && updated == concurrentEdit;
				
				return (pass,
					pass
						? $"PASS  [{sw.ElapsedMilliseconds}ms]"
						: $"FAIL  (stale apply: {apText}) [{sw.ElapsedMilliseconds}ms]");
			}
			finally {
		...
		return new TestGroup("Code Fix Tools", [
			new("roslyn_preview/apply_code_fix: RMCP001 IntPtr → nint",
				() => RunApplyNintFix()),
			new("roslyn_apply_code_fix: rejects stale modified file",
				() => RunRejectStaleFix())
		]);
	}
}
